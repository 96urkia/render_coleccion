...
const I18N = {
  es: {
    ...
    placeholder_busqueda_signatura: "Buscar por signatura / CDU · admite comodines (*)",
    ...
    ayuda_topo: "Cómo exportar el listado topográfico",
    ayuda_catalogo: "Cómo exportar el catálogo (Formato 1 / Cuerpo 1 / Orden 8)",
    ayuda_nunca: "Cómo exportar los ejemplares nunca prestados",
    ayuda_mas: "Cómo exportar los ejemplares más prestados",
  },
  eu: {
    ...
    placeholder_busqueda_signatura: "Bilatu signatura / CDUaren arabera · asterisko (*) onartzen du",
    ...
    ayuda_topo: "Zerrenda topografikoa nola esportatu",
    ayuda_catalogo: "Katalogoa nola esportatu (1 formatua / 1 gorputza / 8 ordena)",
    ayuda_nunca: "Inoiz mailegatu gabeko aleak nola esportatu",
    ayuda_mas: "Gehien mailegatutako aleak nola esportatu",
  },
};
...
// ---------- ayuda en vídeo: miniatura al pasar el ratón / tocar el icono "?" ----------
// Los iconos viven dentro de un <label> que envuelve el <input type="file">, así
// que hay que frenar la propagación del clic o se abriría el selector de archivos.
(function initVideoHelp() {
  const tooltip = $("#video-tooltip");
  const videoEl = $("#video-tooltip-el");
  const captionEl = $("#video-tooltip-caption");
  if (!tooltip || !videoEl) return;

  let activo = null;

  function posicionar(icon) {
  ...
  $$(".help-icon").forEach(icon => {
    icon.addEventListener("mouseenter", () => abrir(icon));
    icon.addEventListener("mouseleave", cerrar);
    icon.addEventListener("focus", () => abrir(icon));
    icon.addEventListener("blur", cerrar);
    icon.addEventListener("click", e => {
      // evita que el clic "burbujee" hasta el <label> y abra el selector de archivos
      e.preventDefault();
      e.stopPropagation();
      if (activo === icon) cerrar(); else abrir(icon);
    });
  });

  document.addEventListener("click", e => {
    if (activo && !e.target.closest(".help-icon") && !e.target.closest(".video-tooltip")) cerrar();
  });
  window.addEventListener("scroll", () => { if (activo) posicionar(activo); }, true);
  window.addEventListener("resize", () => { if (activo) posicionar(activo); });
})();
...
async function cargarTablaSignatura() {
  ...
  const params = {
    ...
    campo: $("#select-campo-busqueda-sig").value,
...
$("#select-campo-busqueda-sig").addEventListener("change", () => {
  const campo = $("#select-campo-busqueda-sig").value;
  $("#input-busqueda-sig").placeholder = t(`placeholder_busqueda_${campo}`);
  state.sigPage = 1;
  cargarTablaSignatura();
});

// Ficha catalográfica de un registro del análisis subido (sin sección de
// sucursales: aquí solo se está analizando la biblioteca propia).
$("#tabla-signatura").addEventListener("click", e => {
  const fila = e.target.closest("tr.fila-clicable");
  if (!fila) return;
  abrirFichaSesion(fila.dataset.idSistema);
});

async function abrirFichaSesion(idSistema) {
  if (!idSistema) return;
  const overlay = $("#ficha-overlay");
  const contenido = $("#ficha-contenido");
  contenido.innerHTML = `<p class="hint">${t("cargando")}</p>`;
  overlay.hidden = false;
  $("#ficha-nav-tabs").hidden = true; // solo hay un panel: no aplica lo de "sucursales"

  try {
    const f = await apiGet("/api/analisis/ficha", { session_id: state.sessionId, id_sistema: idSistema });
    contenido.innerHTML = renderFichaSesion(f);
  } catch (e) {
    contenido.innerHTML = `<p class="ficha-error">${e.message}</p>`;
  }
}

function renderFichaSesion(f) {
  const materias = (f.materias || []).map((m, i) => `${i + 1}. ${m}.`).join(" ");
  let parrafo = `<span class="isbd-titulo">${f.titulo ?? t("ficha_titulo_no_disponible")}</span>`;
  if (f.autor) parrafo += ` / ${f.autor}`;
  if (f.detalle_isbd) parrafo += ` ${f.detalle_isbd}`;

  return `
    <div class="ficha-panel" data-ficha-panel="isbd">
      <div class="isbd-card">
        ${f.signatura ? `<div class="isbd-signatura">${f.signatura}</div>` : ""}
        ...
        <p class="isbd-nota-sistema">${t("ficha_nota_propia")}</p>
      </div>
    </div>
  `;
}
...
// Notas.
// Los subcampos MARC de la red a veces ya incluyen puntuación ISBD (p.ej. un
// $b de editorial que empieza por ": " o un $c de año que empieza por ", "),
// y este párrafo añade la suya propia entre campos. Esto puede producir
// puntuación duplicada (" : :", ", ,", "//", "..") que aquí se colapsa.
function limpiarPuntuacionIsbd(texto) {
  let anterior;
  let actual = texto.replace(/\s{2,}/g, " ");
  do {
    anterior = actual;
    actual = anterior
      .replace(/([.,;:/])\s*\1/g, "$1")
...
function renderIsbdParrafo(f) {
  ...
  partes.push(limpiarPuntuacionIsbd(linea1));

  const fisicaBits = [];
  if (f.descripcion_fisica) fisicaBits.push(f.descripcion_fisica);
  if (f.serie) fisicaBits.push(`(${f.serie})`);
  if (fisicaBits.length) partes.push(limpiarPuntuacionIsbd(fisicaBits.join(" — ")));
