...
def _decodificar_bytes(data: bytes) -> str:
    ...
    "ñ", etc.), no en UTF-8. Decodificar siempre como UTF-8 con
    errors="replace" convertía cada tilde en un carácter de reemplazo (�),
    aunque el fichero original estuviera perfectamente bien. Probamos UTF-8
    estricto primero (por si el usuario lo reexportó como UTF-8) y, si falla,
    caemos a cp1252 (superconjunto de Latin-1, admite errores igualmente).
    """
    if not data:
        return ""
    try:
